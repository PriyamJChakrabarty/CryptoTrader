...
def render_crypto_snapshot(snapshot: dict, experience: str = "") -> str:
    """Render a snapshot dict into a markdown string for PromptBuilder to assemble into a HumanMessage.

    Args:
        snapshot: A dict containing fields such as pair / timestamp / market.* /
                  news.* / onchain.* / macro.*; TechAgent injects the
                  'indicators' field before calling.
        experience: Optional historical experience text; if non-empty, appended
                  at the end (truncated to 4000 chars via sanitize_input).

    Returns:
        The formatted markdown text string.

    Security guarantees:
        - All external content (news headlines / experience) is processed via sanitize_input().
        - Internal prompts (system_prompt / output_schema) are not sanitized (trusted).
        - Follows the spec 015 anti-injection invariant.
    """
