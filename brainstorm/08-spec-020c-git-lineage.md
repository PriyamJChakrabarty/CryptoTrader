...
**Decision**：C — inherit git identity + message trailer `Auto-Generated-By: spec-020c`。

**Rationale**：
- 不引入新 git identity（与 spec 014 commit 风格兼容）
- trailer 机制清晰可 grep（`git log --grep="Auto-Generated-By"`）
- inherit the existing git identity; use the trailer to mark the automated source
...
- **FR-L8**：commit identity inherits the existing git config（不改写）；trailer 唯一标识来源
...
- ❌ GPG sign / multi-author signing（inherit existing git identity）
