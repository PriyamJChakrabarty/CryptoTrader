...
def log_llm_usage(response: Any, *, caller: str) -> None:
    """Log LLM call token usage to structured logging.

    Extracts input_tokens, output_tokens, model_name from
    AIMessage.usage_metadata and records them via structlog under the
    llm_usage event namespace, supporting later aggregation of cost
    reports by time window.

    spec 020a FR-Z7~Z9: also writes 3 OTel span attrs (llm.cache.*)
    and pushes hit_rate to CacheMetricsAggregator for the /metrics endpoint.

    Args:
        response: The AIMessage object returned by the LLM call.
        caller: Caller identifier (e.g. agent_id or function name), used for log location.
    """
    ...
    # spec 020a FR-Z7: extract cache_creation_input_tokens
    ...
    # spec 020a FR-Z18: push hit_rate to in-process aggregator for /metrics endpoint
    try:
        ...
        pass  # aggregator errors do not affect cycle execution
